...
const App = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if(user.username === "user" && user.password === "password"){
      setMessage(`Welcomme, ${user.username}!`);
      setError(false)
    }
    else{
    ...
    <div style={{
      ...
      <div style={{lineHeight: '3', color: "rose", fontSize: "20px"}}>
        <form style={{ display: 'flex', flexDirection: 'column' }} onSubmit={handleSubmit}>
          ...
          <input style={{ fontSize: "10px"}} id="password" name="password" type="password" placeholder="password" value={user.password} onChange={handleChange} required />
          </div>
          <div>
            <button style={{ padding: '10px', margin: '10px 40px 0 30%', border:"2px solid red", borderRadius: "5px"}} type="Submit" >
              Login
            </button>
          </div>
        </form>
